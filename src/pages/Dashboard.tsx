...
const Dashboard = () => {
  ...
  const completedToday = complaints?.filter((t) =>
    ["completed", "closed"].includes(t.status) && new Date(t.updated_at).toDateString() === today
  ).length || 0;
  ...
  const totalCompleted = complaints?.filter(t => ["completed", "closed"].includes(t.status)).length || 0;
